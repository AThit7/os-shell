...
#ifdef STUDENT
    size_t offset = 0; // początek kolejnej ścieżki w path
    size_t paths_len = strlen(path);

    while (offset < paths_len) {
      size_t path_len = strcspn(path + offset, ":");
      char *path_to_execute = strndup(path + offset, path_len);
      strapp(&path_to_execute, "/");
      strapp(&path_to_execute, *argv);

      (void)execve(path_to_execute, argv, environ);
